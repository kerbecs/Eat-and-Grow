"""
------------------------------------------------------------------
                  Eat and Grow Version XI ( The Menu)
------------------------------------------------------------------
"""
...
class Window:
    ...
    window_size = (pygame.display.Info().current_w, round(0.92 * pygame.display.Info().current_h))
    # the previous window size was 1500 x 800, but it was changed to be automatically set for every resolution
    ...
    def __init__(self):
        ...
        self.__buton_color = (235, 12, 12)

        # set the color for a button then the mouse touches it
        self.light_color = (158, 6, 6)

        self.color_button_coords = (round(0.85*self.window_size[0]), round(0.75*self.window_size[1]))
        ...
        # the initial y was 650, but it was changed to be the same for all resolutions
        self.creator_coords_y = round(0.83*self.window_size[1])

        # version specs
        self.__version_text = "Version 1.0.2 Beta"
        self.version_font = "Verdana"
        self.version_size = 20
        self.version_color = (51,51,255)
        # the initial coords were (1260, 735), but they were changed to be the same for all resolutions
        self.version_coords = (round(0.822*self.window_size[0]), round(0.91*self.window_size[1]))
    ...
    def add_text_creator(self):
        ...
        self.__window.blit(text_font.render(self.__creator[1],True,self.creator_color),((self.window_size[0] - text_dimension2[0])/2,
                                                                                        self.creator_coords_y+text_dimension1[1]))
    ...
    def create_buttons(self,color_button_1,color_button_2,color_button3):
        ...
        self.rect1 = [(self.window_size[0] - self.__button_size[0]) / 2, (self.window_size[1]/2-0.9*self.__button_size[1]),
                      self.__button_size[0], self.__button_size[1]]
        self.buton1 = pygame.draw.ellipse(self.__window, color_button_1, self.rect1)
        self.rect2 = [(self.window_size[0] - self.__button_size[0]) / 2,
                      (self.window_size[1] / 2 + 0.9*self.__button_size[1]),
                      self.__button_size[0], self.__button_size[1]]
        self.buton2 = pygame.draw.ellipse(self.__window, color_button_2, self.rect2)

        self.buton3 = pygame.draw.ellipse(self.__window, color_button3, (self.color_button_coords[0], self.color_button_coords[1], self.__button_size[2], self.__button_size[3]))
    ...
    def add_text_on_buttons(self):
        ...
        button_1_center = self.buton1.center
        button_2_center = self.buton2.center
        button_3_center = self.buton3.center

        # add the texts on buttons
        self.__button_1 = self.__window.blit(self.text_font_1.render(self.__button_text[0], True, (255, 255, 255)), ((button_1_center[0]-text1_dim[0]/2)
                                                                                             ,(button_1_center[1]-text1_dim[1]/2)))
        self.__button_2 = self.__window.blit(self.text_font_1.render(self.__button_text[1], True, (255, 255, 255)),
                            ((button_2_center[0] - text2_dim[0]/2), (button_2_center[1] - text2_dim[1]/2)))
    ...
    def verify_mouse_pos(self,close=None):
        ...
        buton1_center = self.__button_1.center
        buton2_center = self.__button_2.center
    ...
    def display_title(self):
        ...
        self.__title = self.__window.blit(title_font.render(self.title_text,True,self.title_color),((self.window_size[0]- title_dimension[0])/2, self.window_size[1]/4 - title_dimension[1]))
