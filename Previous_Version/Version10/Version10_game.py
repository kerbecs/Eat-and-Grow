"""
------------------------------------------------------------------
                  Eat and Grow Version X ( The Game)
------------------------------------------------------------------
"""
import sys
...
class Game:
    ...
    def __init__(self, window, __window_size):
        ...
        self.____player_circle_coords = [(self.__window_size[0]+self.__size_circle_player)/2, (self.__window_size[1]+self.__size_circle_player)/2]
        self.__text = "ME"
        self.__text_size= 15
    ...
    def get_time(self):
        ...
        if ((get_ticks() - Game.last_time) // 1000) - StaticCircles.spawn_time == Game.time_static_circles:
            ...
            Game.create_static_circle(self.____player_circle_coords, self.__size_circle_player)
        if ((get_ticks() - Game.last_time) // 1000) - DangerousCircles.time_spawn == Game.time_dangerous_circles:
            ...
            Game.create_dangerous_circle(self.____player_circle_coords, self.__size_circle_player)
        ...
        if((get_ticks() - Game.last_time) // 1000) - DynamicCircles.time_spawn == Game.time_dynamic_circles:
            ...
            Game.create_dynamic_circle(self.____player_circle_coords, self.__size_circle_player)
    ...
    def draw_player_circle(self):
        ...
        self.__player_circle = pygame.draw.circle(Game.window_surface, self.__player_circle_color, self.____player_circle_coords, self.__size_circle_player)
        self.add_text()
    ...
    def x_y_raport(self):
        ...
        try:
            if round(self.____player_circle_coords[0] + self.__size_circle_player) < self.mouse_pos[0]:
                if round(self.____player_circle_coords[1] + self.__size_circle_player) < self.mouse_pos[1]:
                    self.x_y_rap = (self.mouse_pos[0] - (round(self.____player_circle_coords[0] + self.__size_circle_player)))/(
                            self.mouse_pos[1] - (round(self.____player_circle_coords[1]+self.__size_circle_player)))
                elif round(self.____player_circle_coords[1] - self.__size_circle_player) > self.mouse_pos[1]:
                    self.x_y_rap = (self.mouse_pos[0] - (round(self.____player_circle_coords[0] + self.__size_circle_player)))/(
                            round(self.____player_circle_coords[1] - self.__size_circle_player) - self.mouse_pos[1])
            elif round(self.____player_circle_coords[0] - self.__size_circle_player) > self.mouse_pos[0]:
                if round(self.____player_circle_coords[1] + self.__size_circle_player) < self.mouse_pos[1]:
                    self.x_y_rap = (round(self.____player_circle_coords[0] - self.__size_circle_player) - self.mouse_pos[0]) / (
                        self.mouse_pos[1] - (round(self.____player_circle_coords[1]) + self.__size_circle_player))
                elif round(self.____player_circle_coords[1] - self.__size_circle_player) > self.mouse_pos[1]:
                    self.x_y_rap = (round(self.____player_circle_coords[0] - self.__size_circle_player) - self.mouse_pos[0]) / (
                             round(self.____player_circle_coords[1] - self.__size_circle_player) - self.mouse_pos[1])
    ...
    def move_to_mouse(self):
        ...
        if self.mouse_pos[0] == round(self.____player_circle_coords[0]): # if the mouse and the circle are vertical equal
            if self.mouse_pos[1] > round(self.____player_circle_coords[1] + self.__size_circle_player): # the mouse is below the circle
                self.change_speed(2, 0, 1)
            elif self.mouse_pos[1] < round(self.____player_circle_coords[1] - self.__size_circle_player): # the mouse is above the circle
                self.change_speed(2, 0, -1)
        elif self.mouse_pos[1] == round(self.____player_circle_coords[1]): # if the mouse and the circle are horizontally equal
            if self.mouse_pos[0] > round(self.____player_circle_coords[0] + self.__size_circle_player): # the mouse is on right of the circle
                self.change_speed(2, 1, 0)
            elif self.mouse_pos[0] < round(self.____player_circle_coords[0] - self.__size_circle_player): # the mouse is on left of the circle
                self.change_speed(2, -1, 0)
        elif self.mouse_pos[0] > round(self.____player_circle_coords[0] + self.__size_circle_player): # the mouse is in the second or 4th dial
            if self.mouse_pos[1] > round(self.____player_circle_coords[1] + self.__size_circle_player): # the mouse is in the 4th dial
                if self.x_y_rap > 1:
                   self.change_speed(1, +1, +1)
                elif self.x_y_rap == 1:
                    self.change_speed(2, +1, +1)
            elif self.mouse_pos[1] < round(self.____player_circle_coords[1] - self.__size_circle_player): # the mouse is in the second dial
                if self.x_y_rap > 1:
                    self.change_speed(1, +1, -1)
                elif self.x_y_rap == 1:
                    self.change_speed(2, +1, -1)

        elif self.mouse_pos[0] < round(self.____player_circle_coords[0] - self.__size_circle_player): # the mouse is in the first or third dial
            if self.mouse_pos[1] > round(self.____player_circle_coords[1] + self.__size_circle_player): # the mouse is in the third dial
                if self.x_y_rap > 1:
                    self.change_speed(1, -1, +1)
                elif self.x_y_rap == 1:
                    self.change_speed(2, -1, +1)
            elif self.mouse_pos[1] < round(self.____player_circle_coords[1] - self.__size_circle_player): # the mouse is in the first dial
                if self.x_y_rap > 1:
                    self.change_speed(1, -1, -1)
                elif self.x_y_rap == 1:
                    self.change_speed(2, -1, -1)
    ...
    def move_player_circle(self):
        ...
        self.____player_circle_coords[0] += self.player_circle_speed[0]
        self.____player_circle_coords[1] += self.player_circle_speed[1]

    def verify_bound(self):
        # this method verifies if the player's circle doesn't touch the borders. If it does it, it bounds
        
        if (self.____player_circle_coords[0] <= self.__size_circle_player or self.____player_circle_coords[0] >=
                self.__window_size[0] - self.__size_circle_player
                or self.____player_circle_coords[1] <= self.__size_circle_player or self.____player_circle_coords[1] >=
    ...
    def check_grow_player_circle(self):
        ...
        for static_circle in StaticCircles.static_circle_objects:
            distance_between_circles = sqrt((self.____player_circle_coords[0] - static_circle.coords[0])**2 + (self.____player_circle_coords[1] - static_circle.coords[1])**2)
    ...
    def check_dangerous_player_circle(self):
        ...
        for dangerous_circle in DangerousCircles.dangerous_circle_objects:
            distance_between_circles = sqrt((self.____player_circle_coords[0] - dangerous_circle.coords[0]) ** 2 + (
                        self.____player_circle_coords[1] - dangerous_circle.coords[1]) ** 2)
    ...
    def check_dynamic_circle(self):
        ...
        for dynamic_circle in DynamicCircles.dynamic_circle_objects:
            distance_between_circles = sqrt((self.____player_circle_coords[0] - dynamic_circle.coords[0]) ** 2 + (
                        self.____player_circle_coords[1] - dynamic_circle.coords[1]) ** 2)
    ...
    def add_text(self):
        ...
        Game.window_surface.blit(title_font.render(self.__text, True, self.__text_color),
                                 (self.____player_circle_coords[0]-text_dimension[0]/2, self.____player_circle_coords[1]-text_dimension[1]/2))
        title_font = font.SysFont(self.__text_size_font, round(self.__text_size), bold=True)
        ...
        Game.window_surface.blit(title_font.render(str(round(self.__size_circle_player)), True, self.__text_size_color),
                                 (self.____player_circle_coords[0]-text_dimension[0]/2, self.____player_circle_coords[1]+text_dimension[1]))
